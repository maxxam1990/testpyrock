 ...
 * @package Travelop
 */
if ( ! class_exists( 'Travelop_Instagram_Widget' ) ) {
	class Travelop_Instagram_Widget extends Cherry_Abstract_Widget {
		...
		public function widget( $args, $instance ) {
			...
			if ( ! $photos ) {
				return print $args['before_widget'] . esc_html__( 'No photos. Maybe you entered a invalid hashtag.', 'travelop' ) . $args['after_widget'];
			}
		...
		public function get_photos( $data, $img_counter, $config ) {
			 ...
			 * @var string
			 */
			$order = apply_filters( 'travelop_instagram_widget_order', 'media' );
			...
			foreach ( $nodes as $photo ) {
				...
				if ( in_array( 'caption', $config ) ) {
					$_photo['caption'] = wp_html_excerpt(
						...
						apply_filters( 'travelop_instagram_widget_caption_more', '&hellip;' )
					);
				}
		...
		public function get_tags_url() {
			return apply_filters( 'travelop_instagram_widget_get_tags_url', $this->service_url . 'explore/tags/%s/' );
		}
		/**
		...
		public function get_post_url() {
			return apply_filters( 'travelop_instagram_widget_get_post_url', $this->service_url . 'p/%s/' );
		}
		/**
		 * Clear cache.
		 *
